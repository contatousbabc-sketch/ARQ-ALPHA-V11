...
# Importações dos protocolos CPL individuais
try:
    ...
    from .cpl_protocol_5 import cpl_protocol_5
    HAS_CPL_PROTOCOLS = True
    logger.info("✅ Todos os protocolos CPL (1-5) importados com sucesso")
except ImportError as e:
    HAS_CPL_PROTOCOLS = False
    logger.warning(f"⚠️ Erro ao importar protocolos CPL: {e}")
...
class CPLDevastadorProtocol:
    ...
    async def _fase_1_arquitetura_evento(self, session_id: str, contexto: ContextoEstrategico) -> EventoMagnetico:
        ...
        # Se os protocolos CPL estão disponíveis, usar o Protocol 1
        if HAS_CPL_PROTOCOLS and cpl_protocol_1:
            try:
                logger.info("🎯 Executando CPL Protocol 1 para arquitetura do evento")
                
                # Preparar contexto para o Protocol 1
                contexto_protocol_1 = {
                    'tema': contexto.tema,
                    'segmento': contexto.segmento,
                    'publico_alvo': contexto.publico_alvo,
                    'termos_chave': contexto.termos_chave,
                    'objecoes': contexto.objecoes,
                    'tendencias': contexto.tendencias,
                    'casos_sucesso': contexto.casos_sucesso
                }
                
                # Executar Protocol 1
                resultado_protocol_1 = await cpl_protocol_1.executar_protocolo(contexto_protocol_1, session_id)
                
                if resultado_protocol_1.get('status') == 'concluido':
                    # Converter resultado do Protocol 1 para EventoMagnetico
                    resultados = resultado_protocol_1.get('resultados', {})
                    posicionamento = resultados.get('posicionamento', {})
                    oportunidades = resultados.get('oportunidades', {})
                    
                    return EventoMagnetico(
                        nome=f"Revolução {contexto.tema} - {posicionamento.get('diferencial_competitivo', 'Transformação Definitiva')}",
                        promessa_central=posicionamento.get('proposta_valor', f"Como dominar {contexto.tema} em tempo recorde"),
                        arquitetura_cpls={
                            "cpl1": "A Descoberta Revolucionária - Oportunidade única revelada",
                            "cpl2": "A Prova Impossível - Resultados que desafiam a lógica",
                            "cpl3": "O Método Secreto - Sistema completo revelado",
                            "cpl4": "A Decisão Definitiva - Sua última chance de transformação"
                        },
                        mapeamento_psicologico={
                            "gatilho_principal": "FOMO + Urgência + Exclusividade",
                            "jornada_emocional": "Curiosidade → Choque → Desejo → Ação",
                            "pontos_pressao": oportunidades.get('insights', ["Medo de ficar para trás", "Desejo de transformação"])
                        },
                        justificativa=f"Baseado em análise real de mercado: {resultados.get('analise_mercado', {}).get('tamanho_mercado', 'mercado em crescimento')}"
                    )
                else:
                    logger.warning("⚠️ CPL Protocol 1 falhou, usando método fallback")
                    
            except Exception as e:
                logger.error(f"❌ Erro ao executar CPL Protocol 1: {e}")
                logger.info("🔄 Usando método fallback para arquitetura do evento")
    ...
    async def _fase_2_cpl1_oportunidade(self, session_id: str, contexto: ContextoEstrategico, evento: EventoMagnetico) -> CPLDevastador:
        ...
        if HAS_CPL_PROTOCOLS and cpl_protocol_2:
            try:
                ...
                # Executar Protocol 2
                resultado_protocol_2 = await cpl_protocol_2.executar_protocolo(contexto_protocol_2, session_id)
                
                if resultado_protocol_2.get('status') == 'concluido':
                    # Converter resultado do Protocol 2 para CPLDevastador
                    resultados = resultado_protocol_2.get('resultados', {})
    ...
    async def _fase_3_cpl2_transformacao(self, session_id: str, contexto: ContextoEstrategico, cpl1: CPLDevastador) -> CPLDevastador:
        ...
        if HAS_CPL_PROTOCOLS and cpl_protocol_3:
            try:
                ...
                # Executar Protocol 3
                resultado_protocol_3 = await cpl_protocol_3.executar_protocolo(contexto_protocol_3, session_id)
                
                if resultado_protocol_3.get('status') == 'concluido':
                    # Converter resultado do Protocol 3 para CPLDevastador
                    resultados = resultado_protocol_3.get('resultados', {})
                    cpl_data = resultados.get('cpl_gerado', {})
    ...
    async def _fase_4_cpl3_caminho(self, session_id: str, contexto: ContextoEstrategico, cpl2: CPLDevastador) -> CPLDevastador:
        ...
        if HAS_CPL_PROTOCOLS and cpl_protocol_4:
            try:
                ...
                # Executar Protocol 4
                resultado_protocol_4 = await cpl_protocol_4.executar_protocolo(contexto_protocol_4, session_id)
                
                if resultado_protocol_4.get('status') == 'concluido':
                    # Converter resultado do Protocol 4 para CPLDevastador
                    resultados = resultado_protocol_4.get('resultados', {})
                    cpl_data = resultados.get('cpl_gerado', {})
    ...
    async def _fase_5_cpl4_decisao(self, session_id: str, contexto: ContextoEstrategico, cpl3: CPLDevastador) -> CPLDevastador:
        ...
        if HAS_CPL_PROTOCOLS and cpl_protocol_5:
            try:
                ...
                # Executar Protocol 5
                resultado_protocol_5 = await cpl_protocol_5.executar_protocolo(contexto_protocol_5, session_id)
                
                if resultado_protocol_5.get('status') == 'concluido':
                    # Converter resultado do Protocol 5 para CPLDevastador
                    resultados = resultado_protocol_5.get('resultados', {})
                    cpl_data = resultados.get('cpl_gerado', {})
